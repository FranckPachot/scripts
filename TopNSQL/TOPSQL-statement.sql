...
select * from (  
  select 
  db_name "db",host_name "host",instance_name "inst",to_char(min(begin_interval_time),'yyyy-mm-dd hh24:mi') "begin",to_char(max(end_interval_time),'yyyy-mm-dd hh24:mi') "end"
  --,(cast(max(end_interval_time) as date)-cast(min(begin_interval_time) as date)) snap_microseconds
        ,sum(executions_delta) "total executions"
  ...
  where sql_id='&2' and to_char(begin_interval_time,'yyyy-mm-dd') >= '&4' and executions_delta>0
...
select sql_text from dba_hist_sqltext where sql_id='&2' and (select count(*) from  table(dbms_xplan.display_awr('&2',&3,null,'BASIC')))<=0
/
...
select * from (
  ...
  TEXT 
  from dba_hist_sql_bind_metadata where sql_id='3wap1k01w7a0q'
...
select text from (
  select 
  case when position = min(position)over(partition by sql_id) then '<tr>' end
  || '<td>'||name||' '||datatype_string||' := '||value_string||'</td>'
  ||case when position = max(position)over(partition by sql_id) then '</tr>' end
  TEXT,dense_rank()over(order by snap_id desc) r
  from dba_hist_sqlbind where sql_id='3wap1k01w7a0q'
